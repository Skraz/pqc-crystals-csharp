...
namespace crystals_csharp.crypto.pqc.dilithium
{
    internal class Poly
    {
        ...
        public void PointwiseMontgomery(Poly v, Poly w)
        {
            ...
            for (i = 0; i < N; ++i)
            {
                Coeffs[i] = Reduce.MontgomeryReduce(v.Coeffs[i] * (long)w.Coeffs[i]);
            }
        }
        ...
        public void PolyT1Unpack(byte[] a, int off)
        {
            ...
            for (i = 0; i < N / 4; ++i)
            {
                Coeffs[4 * i + 0] = ((a[off + 5 * i + 0] & 0xFF) >> 0 | (a[off + 5 * i + 1] & 0xFF) << 8) & 0x3FF;
                Coeffs[4 * i + 1] = ((a[off + 5 * i + 1] & 0xFF) >> 2 | (a[off + 5 * i + 2] & 0xFF) << 6) & 0x3FF;
                Coeffs[4 * i + 2] = ((a[off + 5 * i + 2] & 0xFF) >> 4 | (a[off + 5 * i + 3] & 0xFF) << 4) & 0x3FF;
                Coeffs[4 * i + 3] = ((a[off + 5 * i + 3] & 0xFF) >> 6 | (a[off + 5 * i + 4] & 0xFF) << 2) & 0x3FF;
            }
        }
